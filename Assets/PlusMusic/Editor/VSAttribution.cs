...
namespace UnityEditor.VSAttribution.PlusMusic
{
	public static class VSAttribution
	{
		...
		static void Init()
		{
			PlusMusic_DJ.VSASendAttributionEvent = PlusMusicDelegate;
		}

		public static bool PlusMusicDelegate(string actionName, string partnerName, string customerUid)
		{
			Debug.Log($"PlusMusicDelegate:SendAttributionEvent(): {actionName}, {partnerName}, {customerUid}");
			AnalyticsResult result = SendAttributionEvent(actionName, partnerName, customerUid);
			Debug.Log($"PlusMusicDelegate:SendAttributionEvent(): result = {result}");

			return (AnalyticsResult.Ok == result);
		}
